...
const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    version: "1.4.0",
    title: {
      en: "WeChat Pay, Knowledge Base RAG, Leaderboard & Sales Channels",
      zh: "微信支付、知识库 RAG、排行榜与销售渠道",
      "zh-TW": "微信支付、知識庫 RAG、排行榜與銷售渠道",
      fr: "WeChat Pay, RAG base de connaissances, classement et canaux de vente",
      ko: "WeChat Pay, 지식 기반 RAG, 리더보드 및 판매 채널",
    },
    items: {
      en: [
        "WeChat Pay integration — Chinese users on yeoso.com can pay via WeChat QR code for Growth and Scale plans.",
        "Knowledge Base RAG for all plans — Upload documents, PDFs, or URLs and AI agents use them as context for content generation and email outreach. Powered by pgvector embeddings + LlamaIndex.",
        "AI Model Leaderboard — Compare performance rankings of LLM models (Claude, GPT, Gemini, Llama, Qwen) with real benchmark data.",
        "Xianyu marketplace — New sales channel integration for the Chinese secondhand market.",
        "Firecrawl integration — Web crawling tool for extracting website content into the knowledge base.",
        "SMTP support — Configure custom SMTP servers as an alternative to Brevo/SendGrid for email delivery.",
        "Agent conversation history — AI chat conversations are now persisted and can be resumed.",
        "Agent loop execution — Agents can run multiple task steps in sequence automatically.",
        "Customer search improvements — Better contact search and enrichment with data source verification.",
        "Homepage lead capture — Registration form on landing page to capture trial leads.",
        "Email verification — Validate email addresses before sending to reduce bounce rates.",
        "Pricing plan updates — Refined plan features and pricing display for Growth, Scale, and Enterprise tiers.",
        "TikTok content posting — Connect TikTok account and publish videos directly from AutoClaw.",
        "Chat widget upgrade — Better error handling, model selection, and AIGC content generation in AI chat.",
        "GitHub Actions — Automated branch sync workflow (yeoso → main) for continuous deployment.",
        "Model consumption labels — Clear display of which models are free vs. paid per request.",
        "Multiple bug fixes — Chat errors, deployment issues, cron job reliability, i18n translation keys.",
      ],
      zh: [
        "微信支付集成 — yeoso.com 中文用户可通过微信二维码支付 Growth 和 Scale 计划。",
        "知识库 RAG 全面开放 — 上传文档、PDF 或 URL，AI 智能体使用它们作为内容生成和邮件营销的上下文。基于 pgvector 嵌入 + LlamaIndex。",
        "AI 模型排行榜 — 比较 LLM 模型（Claude、GPT、Gemini、Llama、Qwen）的性能排名和真实基准数据。",
        "闲鱼销售渠道 — 新增中国二手市场销售渠道集成。",
        "Firecrawl 集成 — 网页爬取工具，将网站内容提取到知识库中。",
        "SMTP 支持 — 配置自定义 SMTP 服务器作为 Brevo/SendGrid 的替代方案。",
        "智能体对话历史 — AI 对话记录持久化，可随时恢复。",
        "智能体循环执行 — 智能体可自动连续执行多个任务步骤。",
        "客户搜索优化 — 改进联系人搜索和数据源验证的数据充实功能。",
        "首页线索捕获 — 落地页注册表单，捕获试用线索。",
        "邮箱验证 — 发送前验证邮箱地址，降低退信率。",
        "定价方案更新 — 优化 Growth、Scale、Enterprise 计划的功能展示和价格。",
        "TikTok 内容发布 — 连接 TikTok 账户，直接发布视频。",
        "对话组件升级 — 更好的错误处理、模型选择和 AIGC 内容生成。",
        "GitHub Actions — 自动化分支同步工作流（yeoso → main），持续部署。",
        "模型消耗标签 — 清晰显示每次请求哪些模型免费、哪些收费。",
        "多项 Bug 修复 — 对话错误、部署问题、定时任务可靠性、国际化翻译键。",
      ],
      "zh-TW": [
        "微信支付整合 — yeoso.com 中文用戶可透過微信 QR 碼支付。",
        "知識庫 RAG 全面開放 — 上傳文件、PDF 或 URL，AI 智能體使用它們作為內容生成和郵件行銷的上下文。",
        "AI 模型排行榜 — 比較 LLM 模型的效能排名和真實基準資料。",
        "閒魚銷售渠道 — 新增中國二手市場銷售渠道整合。",
        "Firecrawl 整合 — 網頁爬取工具，將網站內容提取到知識庫中。",
        "SMTP 支援 — 配置自訂 SMTP 伺服器。",
        "智能體對話歷史、循環執行、客戶搜尋優化。",
        "首頁線索捕獲、郵箱驗證、定價方案更新。",
        "TikTok 內容發佈、對話元件升級、GitHub Actions 自動部署。",
        "模型消耗標籤、多項 Bug 修復。",
      ],
      fr: [
        "Intégration WeChat Pay — Paiement via QR code WeChat sur yeoso.com.",
        "RAG base de connaissances pour tous — Documents, PDFs et URLs utilisables comme contexte par les agents IA.",
        "Classement des modèles IA — Comparaison des performances LLM avec données de benchmark.",
        "Canal de vente Xianyu — Intégration du marché secondaire chinois.",
        "Intégration Firecrawl, support SMTP, historique de conversation, exécution en boucle des agents.",
        "Capture de leads, vérification email, mise a jour des tarifs, publication TikTok.",
        "GitHub Actions, labels de consommation des modeles, corrections de bugs multiples.",
      ],
      ko: [
        "WeChat Pay 통합 — yeoso.com에서 QR 코드 결제 가능.",
        "지식 기반 RAG — 문서, PDF, URL 업로드 후 AI 에이전트가 컨텍스트로 활용.",
        "AI 모델 리더보드 — LLM 모델 성능 비교 및 벤치마크 데이터.",
        "Xianyu 판매 채널, Firecrawl 통합, SMTP 지원, 대화 기록.",
        "에이전트 루프 실행, 고객 검색 개선, 홈페이지 리드 캡처.",
        "이메일 인증, 가격 업데이트, TikTok 게시, 채팅 위젯 개선.",
        "GitHub Actions, 모델 소비 라벨, 다수의 버그 수정.",
      ],
    },
  },
  {
