...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const locationIds = Array.isArray(body.locationIds) ? body.locationIds.map(String).filter(Boolean) : [];
  ...
  const result = await setCampaignLocations(rows[0].platform_campaign_id as string, locationIds);
  if (!result.success) {
    return NextResponse.json({ error: "Failed to update locations", details: result.error }, { status: 502 });
  }
  ...
  logAudit({
    ...
    details: { sub_action: "update_locations", count: locationIds.length, locationIds },
    ipAddress: ip,
  });
