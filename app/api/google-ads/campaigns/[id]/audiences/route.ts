import { NextRequest, NextResponse } from "next/server";
import { auth0 } from "@/lib/auth0";
import { getDb } from "@/lib/db";
import { logAudit } from "@/lib/audit";
import { checkRateLimit } from "@/lib/rate-limit";
import { setCampaignAudiences, type AudienceApiType, type AudienceCriterionInput } from "@/lib/google-ads";
import { resolveOrgId } from "@/lib/credits";
import { isReadOnlyUserId } from "@/lib/roles-server";

export const dynamic = "force-dynamic";

const ALLOWED_TYPES = new Set<AudienceApiType>([
  "AGE_RANGE", "GENDER", "PARENTAL_STATUS", "INCOME_RANGE",
  "USER_INTEREST", "TOPIC", "USER_LIST", "CUSTOM_AUDIENCE",
]);

function getIp(req: NextRequest): string {
  return req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown";
}

function parseCriteria(raw: unknown): AudienceCriterionInput[] | null {
  if (!Array.isArray(raw)) return null;
  const out: AudienceCriterionInput[] = [];
  for (const item of raw) {
    if (!item || typeof item !== "object") return null;
    const r = item as Record<string, unknown>;
    const apiType = String(r.apiType || "");
    const value = String(r.value || "");
    if (!ALLOWED_TYPES.has(apiType as AudienceApiType)) return null;
    if (!value) return null;
    out.push({ apiType: apiType as AudienceApiType, value, negative: !!r.negative });
  }
  return out;
}

export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const ip = getIp(req);
  if (!checkRateLimit(ip, { limit: 30, windowMs: 60_000 })) {
    return NextResponse.json({ error: "Too many requests" }, { status: 429 });
  }
  const session = await auth0.getSession();
  if (!session?.user?.email) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { id } = await params;
  const campaignId = Number(id);
  if (!Number.isFinite(campaignId)) return NextResponse.json({ error: "Invalid id" }, { status: 400 });

  const body = await req.json().catch(() => ({}));
  const criteria = parseCriteria(body.audiences);
  if (!criteria) return NextResponse.json({ error: "Invalid audiences payload" }, { status: 400 });

  const sql = getDb();
  const userEmail = session.user.email as string;
  const users = await sql`SELECT id FROM users WHERE email = ${userEmail}`;
  if (users.length === 0) return NextResponse.json({ error: "User not found" }, { status: 404 });
  const userId = users[0].id as number;
  if (await isReadOnlyUserId(sql, userId)) {
    return NextResponse.json({ error: "Read-only account — writes are disabled" }, { status: 403 });
  }

  const requestedOrgId = body.orgId ? Number(body.orgId) : undefined;
  const orgId = await resolveOrgId(sql, userId, requestedOrgId);
  if (!orgId) return NextResponse.json({ error: "No organization found" }, { status: 400 });

  const rows = await sql`
    SELECT platform_campaign_id, closed FROM campaigns
    WHERE id = ${campaignId} AND org_id = ${orgId} AND platform = 'google'
  `;
  if (rows.length === 0) return NextResponse.json({ error: "Campaign not found" }, { status: 404 });
  if (rows[0].closed) return NextResponse.json({ error: "Campaign is closed" }, { status: 409 });

  const result = await setCampaignAudiences(rows[0].platform_campaign_id as string, criteria);
  if (!result.success) {
    return NextResponse.json({ error: "Failed to update audiences", details: result.error }, { status: 502 });
  }

  logAudit({
    userId, userEmail,
    action: "google_ads.create_campaign",
    resourceType: "campaign", resourceId: campaignId,
    details: { sub_action: "update_audiences", count: criteria.length, types: criteria.map((c) => c.apiType) },
    ipAddress: ip,
  });

  return NextResponse.json({ success: true, count: criteria.length });
}
